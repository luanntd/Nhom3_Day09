...
# ─────────────────────────────────────────────
# Test & Demo (or FastAPI HTTP Server)
# ─────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        # Khởi chạy HTTP Server bằng FastAPI để đạt Bonus +2
        try:
            ...
            def call_tool(req: ToolCallRequest):
                result = dispatch_tool(req.tool, req.input)
                if "error" in result and not isinstance(result["error"], dict):
                    # For format conformity
                    pass 
                return result

            print("🚀 Khởi chạy MCP HTTP Server (tại cổng 8080)...")
            uvicorn.run(app, host="0.0.0.0", port=8080)
        except ImportError:
            print("⚠️ Cần cài đặt fastapi và uvicorn: pip install fastapi uvicorn pydantic")
            print("Chạy `python mcp_server.py --test` để mock in-process.")
