...
# Notes Sprint 1: Điều chỉnh chunk size và overlap theo quyết định của nhóm
...
def preprocess_document(raw_text: str, filepath: str) -> Dict[str, Any]:
    ...
    for line in lines:
        if not header_done:
            # Note: Parse metadata từ các dòng "Key: Value"
    ...
    # Note: Thêm bước normalize text nếu cần
...
def chunk_document(doc: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Note: Implement chunking theo section heading
...
def _split_by_size(
    ...
    # Note: Implement split theo paragraph với overlap
    # Gợi ý:
    ...
    while start < len(text):
        ...
        # Note: Tìm ranh giới tự nhiên gần nhất (dấu xuống dòng, dấu chấm)
...
def get_embedding(text: str) -> List[float]:
    ...
    Đã chọn: OpenAI Embeddings API (text-embedding-3-small)
    Cần OPENAI_API_KEY trong .env file
    """
    from openai import OpenAI
    
    # Cache client để tái sử dụng
    if not hasattr(get_embedding, '_client'):
        api_key = os.getenv("OPENAI_API_KEY")
        if not api_key:
            raise ValueError("OPENAI_API_KEY not found in .env file")
        get_embedding._client = OpenAI(api_key=api_key)
    
    client = get_embedding._client
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    print("Khởi tạo OpenAI embedding client...")
    get_embedding("test")  # Initialize client once
...
def inspect_metadata_coverage(db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    Note: Implement sau khi build_index() hoàn thành.
    """
    try:
        import chromadb
        client = chromadb.PersistentClient(path=str(db_dir))
...
if __name__ == "__main__":
    ...
    print("  2. Implement phần Note trong build_index()")
